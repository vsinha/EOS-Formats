...
float clSliFile::getLayerPos(int PartIndex, int LayerIndex)
{
	if (PartIndex != 0)
		return -1;
	if ((LayerIndex < 0) || (LayerIndex >= m_FileHead.LayerCount))
		return -1;
...
{
	if (index2Check < *minIndex)
		return -1;
	if (index2Check > *maxIndex)
		return +1;
...
int clSliFile::getLayerIndexByPos(int PartIndex, float LayerPos)
{
	...
	if (LayerPos < m_IndexTable[minIndex].layerPos)
		return -1;
	if (LayerPos > m_IndexTable[maxIndex].layerPos)
		return -1;

	int chIndex = m_currentLayerIndex;

	//- may the current didn't change?
	if (checkLayerPos(&minIndex, &maxIndex, m_currentLayerIndex + 0, LayerPos) == 0)
	{
		m_currentLayerIndex += 0;
		return m_currentLayerIndex;
	}
	if (checkLayerPos(&minIndex, &maxIndex, m_currentLayerIndex + 1, LayerPos) == 0)
	{
	...
	if (checkLayerPos(&minIndex, &maxIndex, m_currentLayerIndex + 2, LayerPos) == 0)
	{
